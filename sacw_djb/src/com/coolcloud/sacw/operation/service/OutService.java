...
public class OutService {
	...
	public Integer out(OutVo vo) {
		...
		property.setPropertyStatus("已出库");
		property.setPropertyStatusCode("9911000000012");
		property.setRemake("该财物已经出库");
	...
	@Transactional
	public Integer batchout1(OutVo vo) {
		...
		if (!propertyIds.contains(",")) {
			...
			// 发生借调操作的时候财物的状态会发生变化
			/*
			 * File1Example example = new File1Example(); example.setExchangeId(tempId);
			 * List<File1> file1s = file1Service.getByExample(example);
			 */
			// Assert.isTrue(file1s.size() != 0, "未找到对应附件");
			property.setPropertyStatus("已出库");
	...
	@Transactional
	public Integer batchOut(OutVo vo) {
		...
		if (!propertyIds.contains(",")) {
			...
			exchange.setProcessCode("1011");
			exchange.setNodeCode("1011001");
			...
			ep.setPropertyStatus("已出库");
			ep.setPropertyStatusCode("9911000000012");
			...
			for (int i = 0; i < id.length; i++) {
				 ...
				 * listProperty.add(property); //把获取到的案件的id放在一个集合中 for(int
				 * j=0;j<listProperty.size();j++) { List<String> listString=new
				 * ArrayList<String>(); listString.add(listProperty.get(i).getId()); }
				 */
			...
			for (int a = 0; a < listString.size(); a++) {
				...
				/*
				 * String
				 * caseName=caseService.findCaseByPropertyId(listString.get(a)).getCaseName();
	...
	public boolean checkPropertiesState(List<Property> properties) {
		for (Property property : properties) {
			boolean areYouOk = "9911000000011".equals(property.getPropertyStatusCode());
			if (!areYouOk) {
				return false;
			}
		}
	...
	public List<Property> getOutList(PropertyVo vo) {
		...
		switch (status) {
		case 0:
			// vo.setPropertyStatusCode("9911000000004");
			PageHelper.startPage(page, rows);
			property = propertyService.selectNotSendPropertyByManyConditions(vo);
			break;
		case 1:
			vo.setPropertyStatusCode("9911000000012");
			PageHelper.startPage(page, rows);
...
*/
	...
	@Transactional(readOnly = true)
	public List<Case> getOutList(PropertyVo vo) {
