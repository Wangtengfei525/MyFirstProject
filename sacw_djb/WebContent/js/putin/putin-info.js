$(function() {
    ...
    $.ajax({
    	url:locat+"/newcase/getcaseinfo?id="+id,
    	type:'get',
    	dataType:'json',
    	success:function(data){
    		$("#caseName").val(data.caseName);
    		$("#sendUnitName").val(data.organizerName);
   ...
   /* $.ajax({
    	url : locat + '/newproperty/getpropertyinfo?property_id=' + id,
    	dataType:'json',
    	success:function(data){
    		console.log(JSON.stringify(data));
    	},
...
function printDetail(id) {
    window.open(locat + '/put-in/print-detail?id=' + id);
}
...
function printReceipt(id) {
    window.open(locat + '/put-in/print-receipt?id=' + id);
}
 ...
 * @returns
 */
function putin(id, tempId) {
    top.layer.confirm('确认入库吗？', function(index) {
        top.layer.close(index);
        index = top.layer.load();
        $.post(locat + '/put-in/put-in-storage', {
            id : id,
            tempId : tempId,
