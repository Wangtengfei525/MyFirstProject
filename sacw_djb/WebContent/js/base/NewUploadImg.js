...
function submitPicture(url,data) {
	var uploadID = getParameter(window.location.search, 'uploadID');
	if (!uploadID) {
		top.layer.alert('未指定交换记录id，请联系管理员！');
	}
var newurl=locat + url+"?exchangeId="+uploadID;
	if(data){
...
function imgDisplay(obj) {
	...
	var imgHtml = '<div style="width: 100%;height: 100%;overflow: auto;background: rgba(0,0,0,0.5);text-align: center;position: fixed;top: 0;left: 0;z-index: 1000;"><img src=' + src + ' style="margin-top: 100px;width: 70%;margin-bottom: 100px;"/><p style="font-size: 50px;position: fixed;top: 30px;right: 30px;color: white;cursor: pointer;" onclick="closePicture(this)">×</p></div>'
	$('body').append(imgHtml);
}
//关闭
