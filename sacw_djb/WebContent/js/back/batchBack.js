$(function() {
  ...
  /* $('#btn-print-detail').click(function() {
    ...
    $('#table-property').bootstrapTable({
        ...
        height : 330,
        url : locat + '/property/findNotBackPropertyByCaseId?id='+id,
        columns : [
...
function printDetail() {
    ...
    window.open(locat + '/back/print-detail?propertyIds=' + propertyIds);
}
...
function printReceipt() {
	...
	if (selections.length === 0) {
	    ...
	    var  propertyIds=idsArr.toString();
    window.open(locat + '/back/print-receipt?propertyIds=' + propertyIds);
}
...
/**
 * 
 * @param propertyIds    选中的所有财物的id组成的字符串,财物id之间用逗号隔开
 * @param tempId    
 * @returns
 */


function batchBack(tempId) {
	/*alert("选中财物的id是"+propertyIds);
	alert(tempId);
	*/
	var selections = $('#table-property').bootstrapTable('getSelections');
	if (selections.length === 0) {
	    top.layer.msg('请选择要归还的财物！', {
	        icon : 2
	    });
	    return;
	}
